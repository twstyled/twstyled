...
function styled<TConstructor extends FC<any>>(
  tag: TConstructor extends FC<infer T>
...
type StyledComponent<T> = StyledMeta &
  (T extends FC<any> ? T : FC<T & { as?: React.ElementType }>)
...
type ComponentStyledTag<T> = <
  OwnProps = {},
  TrgProps = T extends FC<infer TProps> ? TProps : T
>(
  strings: TemplateStringsArray,
...
export default (process.env.NODE_ENV !== 'production'
  ? new Proxy(styled, {
      get(o: any, prop: keyof JSX.IntrinsicElements) {
        return o(prop)
      }
    })
  : styled) as Styled
